...
@media (prefers-color-scheme: dark) {
  ...
  header#title-block-header { border-bottom-color: #4a4a4a; }
        ...
        try:
            if not versions:
                ...
                self.pandoc_convert(tex_file, output_dir / "latest.html",
                                  article_name, datetime.now().strftime("%Y-%m-%d"),
    ...
    def generate_collection_index(self, collection_name: str, tex_files: List[Path]):
        ...
        html.append('</ol>')
        html.append('<div class="nav"><a href="../../index.html">← Back to main index</a></div>')
        html.append('</body></html>')
    ...
    def generate_main_index(self):
        ...
        html.append(f'<p class="date">Last updated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>')
        ...
        if self.collections_dir.exists():
            ...
            if collections:
                ...
                for collection_dir in collections:
                    collection_name = collection_dir.name
                    tex_count = len(list(collection_dir.rglob("*.tex")))
                    html.append(f'<li>')
